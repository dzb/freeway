...
- Extension points - `binder.contribute(Route.class).add(...)` and ordered `add(id, value).before/after(...)`. Consume contributions with explicit `@Inject`: `@Inject List<V>` (all contributions, ordered) or `@Inject Map<String, V>` (named contributions, keyed by id). Unannotated `List`/`Map` parameters resolve as ordinary services, and `@Inject("id")` prefers a bound service over contributions. `Extension<V>` is a framework-internal handle and is rejected at injection time.
