...
- `XDefault` is the framework's default choice — replaceable; extension modules bind an alternative via `.primary()` (e.g. `PoolDefault` vs a Hikari-backed pool): `AppRuntimeDefault`, `JsonCodecDefault`, `PoolDefault`, `FlowDriverDefault`, `ExchangeMetaDefault`, `FlowEngineDefault`.
- `XImpl` is a definitive implementation that is not "the" default of a shared role — either a framework-internal piece the container assembles (`DatabaseImpl`, `QueryImpl`, `ContainerImpl`), or one of several implementations that legitimately coexist per use-site and are each actually used (e.g. `PooledConnectionImpl` vs a pool adapter's own connection type).
- **Choosing Default vs Impl**: the deciding question is whether the type is *the one in-effect implementation of its role* — the default every consumer gets, even when the framework wires it concretely — or one of *several implementations genuinely in use*. One in effect → `XDefault` (how the framework references it is not disqualifying: `ExchangeMetaDefault` is held concretely by the HTTP context yet remains the default exchange metadata; `FlowEngineDefault` is the default flow engine). Several coexist per use-site, or the class is a container-internal assembly piece with no external role → `XImpl` (e.g. `HttpContextImpl`, `CoercerImpl`, `PooledConnectionImpl`). A `*Impl` that only the owning module references may live in `internal`; one that other modules or adapters construct stays in the feature package.
