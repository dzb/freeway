...
> 默认类名（`TransportSecurity.NONE` 常量 / `TransportSecurityDefault`）、
...
- **传输加密（mTLS）**：`TransportSecurity` 抽象（`rpc/` 包）。core 用
  ...
  `TransportSecurityDefault`（rpc/，`fromKeyStore`）。能力由运行时配置
...
public final class CloudConfigKeys {
    ...
    public static final long   EVENT_HANDSHAKE_TIMEOUT_MS_DEFAULT = 10_000;
    ...
    public static final long   EVENT_BACKOFF_MAX_MS_DEFAULT  = 30_000;
}
```
...
- core 交付的默认实现（均可被替代绑定覆盖）：
  ...
  `TransportSecurity.NONE` 常量与 `TransportSecurityDefault`（`rpc/`，
