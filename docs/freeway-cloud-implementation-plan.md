...
> Phase 8（freeway-ext 云后端适配器）**未开工、无排期**——freeway-ext
> 目前不含任何 cloud 适配器，接入协议与现状见定稿 §8/§12.1（2026-09-04
> 更新）。本文为过程性任务清单，其中早期条目（`config` 包、`internal/`
> 放置、`ServiceId` 公开类型等）不代表最终落地，以定稿维护说明与源码为准。
